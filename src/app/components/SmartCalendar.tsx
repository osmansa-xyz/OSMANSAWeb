...
export function SmartCalendar() {
  ...
  // Logika apakah sebuah event sudah lewat (berdasarkan tanggal hari ini)
                  ...
                  {weeks.map((week, wi) => (
                    <div key={wi} className="grid grid-cols-7 gap-2">
                      {week.map((day, di) => {
                        ...
                        return (
                          <div key={di} className={`aspect-square flex flex-col items-center justify-center rounded-xl transition-all border-2 
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-3xl shadow-lg border-2 border-gray-100 p-6 sticky top-6">
              <div className="flex items-center gap-2 mb-6 text-gray-900">
                 <CalendarIcon className="w-5 h-5 text-[#8FB935]" />
                 <h4 className="text-xl font-bold">Daftar Event</h4>
              </div>
              ...
              <div className="space-y-4">
                {currentMonthEvents.length > 0 ? (
                  currentMonthEvents.map((event, i) => {
                    ...
                    const isTodayEvent = isToday(new Date(event.startDate).getDate());

                    return (
                      <div key={i} className={`p-4 rounded-xl border transition-all ${isTodayEvent ? 'bg-blue-50 border-blue-200' : 'bg-gray-50 border-gray-100'}`}>
                        <div className="flex items-start gap-3">
                          <div className={`w-12 h-12 rounded-lg flex flex-col items-center justify-center flex-shrink-0 text-white ${isTodayEvent ? 'bg-blue-500' : isPast ? 'bg-gray-400' : 'bg-[#8FB935]'}`}>
                          ...
                          <div className="flex-1">
                            <div className="flex justify-between items-start">
                              <h5 className={`font-bold leading-tight mb-1 ${isPast ? 'text-gray-500' : 'text-gray-900'}`}>{event.title}</h5>
                              {isTodayEvent && <span className="text-[9px] bg-blue-500 text-white px-1.5 py-0.5 rounded font-bold animate-pulse">HARI INI</span>}
                              {isPast && !isTodayEvent && <span className="text-[9px] bg-gray-200 text-gray-500 px-1.5 py-0.5 rounded font-bold">BERLALU</span>}
                            </div>
